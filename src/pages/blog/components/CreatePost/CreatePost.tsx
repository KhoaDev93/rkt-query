import { useAddPostMutation, useGetPostQuery, useUpdatePostMutation } from 'pages/blog/blogAxios.service'
...
const CreatePost = () => {
  ...
  const { data, isLoading, isFetching } = useGetPostQuery(postId, { skip: !postId })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (postId) {
        ...
        const dataWithId: Post = { ...formData, id: Date.now().toString() }
        await addPost(dataWithId).unwrap()
      }
      setErrorForm(null)
  ...
  return (
    <form onSubmit={handleSubmit} onReset={handleCancelEditing}>
      ...
      <div className='mb-6'>
        ...
        />
        {errorForm?.publishDate && <p>{errorForm?.publishDate}</p>}
      </div>
